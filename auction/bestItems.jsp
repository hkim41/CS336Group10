...
<p> Best selling items are the items that sold the most at auction</p>
	...
	try {
		...
		String str = "SELECT p.product_id, p.category, p.brand, p.color, count(*) FROM auction a, product p WHERE a.status='close' and a.current_bid>=a.min_price and a.product_id=p.product_id group by p.product_id order by count(*) desc limit 5";
