...
<body>
	...
	try {
		...
		int product_id = Integer.parseInt(request.getParameter("item_id"));
		...
		String insert = "INSERT INTO auction (item_id, seller, new_bid_increment, min_price, price, status, start_date, end_date, winner, current_bid)"
		+ "VALUES(?, ?, ?, ?, ?, ?, now(), ?, ?, ?)";
		...
		String str = "INSERT INTO alerts(username, alert_message, item_id, auction_id)" + "VALUES(?, ?, ?, ?)";
		ps = conn.prepareStatement(str);
		ps.setString(1, result2.getString("username"));
		ps.setString(2, "The item " + result2.getString("item_id") + ", is available in auction with id: "
				+ auction_id + ". Go bid to own it!");
		ps.setInt(3, result2.getInt("item_id"));
